# #Solver is to be run with the options and solve a board.
class Solver
  ...
  def self.solve_sudoku(board_state)
  ...
  def self.is_safe?(board_state, column, row, num)
  ...
  def self.find_next_unassigned_location(board_state)
